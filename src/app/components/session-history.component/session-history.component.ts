...
import { CharacterData } from '../../services/character.service';
...
export class SessionHistoryComponent implements OnChanges {
  ...
  expandedPlayerStats: { [key: string]: boolean } = {};
  usernameCache: { [uid: string]: string } = {};
  ...
  togglePlayerStats(sessionId: string, uid: string): void {
    const key = sessionId + uid;
    this.expandedPlayerStats[key] = !this.expandedPlayerStats[key];
  }
  ...
  getSnap(s: any, uid: string): CharacterData {
    return s.playerSnapshots[uid] as CharacterData;
  }
